...
dirs = ["fastqc", "hisat2", "cutadapt", "stringtie", "ballgown"]
# print(dirs[0])

for i in dirs:
	if os.path.exists(os.path.join("results", i)):
		print("directory "+i+" exists in results directory")
	else:
		print("creating "+i+" directory")
		os.mkdir(os.path.join("results", i))

# store samples in raw data directory
data_dir = "/home/suresh/Desktop/raw_data"
# print(data_dir)
samples = glob_wildcards(data_dir + "/{sample}_r1.fastq.gz").sample
# print(samples)
# print(data_dir)
# print(config['ref']['annotation'])
